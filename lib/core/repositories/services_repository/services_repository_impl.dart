...
class ServicesRepositoryImpl implements ServicesRepository {
  ...
  Future<List<Service>> fetchServices() async {
    final servicesJsonData =
        await _firebaseService.getDataCollection(FirebasePaths.recommended_services);        
    final services = servicesJsonData
  ...
  Stream<QuerySnapshot> fetchServicesAsStream() {
    return _firebaseService.streamDataCollection(FirebasePaths.recommended_services);
  }

  Future<Service> getServiceById( String id) async {
    var doc = await _firebaseService.getDocumentById(FirebasePaths.recommended_services, id);
    return  Service.fromMap(doc.data) ;
  }


  Future removeService(String id) async{
     await _firebaseService.removeDocument(FirebasePaths.recommended_services, id) ;
     return ;
  }
  Future updateService(Service data,String id) async{
    await _firebaseService.updateDocument(FirebasePaths.recommended_services, data, id) ;
    return ;
  }

  Future addService(Service data) async{
    var result  = await _firebaseService.addDocument(FirebasePaths.recommended_services, data.toJson()) ;
